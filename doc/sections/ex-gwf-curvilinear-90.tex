...
MODFLOW 6 vertex grids specify a set of vertices that define the boundary of each model cell. Two vertices in the same horizontal plain are connected by a straight line to represent the face of the model cell. A set of vertices can represent a curvilinear grid \citep{romero2006grid} by a piecewise linear representation of the curved surfaces. Curvilinear grids can represent a radial model or replicate a curved flow pattern with a major change in the dominate flow path. For example, a 90$^{\circ}$ curvilinear grid can change the dominate flow path from the x-direction to the y-direction along the same model ``column''.  This example demonstrates the capability of the Discretization by Vertices (DISV) Package to represent a curvilinear grid.
