...
for k in range(nlay):
    for j in range(1,ncol-1):               # outside columns are not UZF cells
        ...
        transp = (iuzno, 0.)                      # sets the starting concentration
...
def build_model(sim_name, mixelm=0, silent=False):
    if config.buildModel:
        ...
        cnc0_left = [(k, 0,      0, 0., 1) for k in range(0, nlay)]
        cnc0_right = [(k, 0, ncol-1, 0., 1) for k in range(0, nlay)]
        ...
        cuzinf[0, 15:25] = 1.
        uzt = flopy.mt3d.Mt3dUzt(mt, iuzfbnd=iuzfbnd, iet=0, 
                                 cuzinf=cuzinf)
        ...
        for k in np.arange(nlay):
            if botm[k] <= strt[k, 0, 0]:
                #              (l, r,      c),              head, conc
                chdspd.append([(k, 0,      0), strt[k, 0,      0], 0.])
                chdspd.append([(k, 0, ncol-1), strt[k, 0, ncol-1], 0.])
        chdspd = {0: chdspd}
...
def plot_results(mf2k5, mt3d, mf6, idx, ax=None):
    if config.plotModel:
        ...
        ucobj_mf6 = flopy.utils.binaryfile.HeadFile(fname, precision='double', text='CONCENTRATION')
        uzconc_mf6 = ucobj_mf6.get_alldata()
        # Now get the concentrations of the saturated zone in MF6
        fname = 'gwt-uzt-2d-mf6.ucn'
        fname = os.path.join(mf6_out_path, fname)
        satconcobj_mf6 = flopy.utils.binaryfile.HeadFile(fname, precision='double', text='CONCENTRATION')
        ...
        cs1 = mx.contour_array(conc_mt3d[-1], colors="k",
                               linestyles="-", levels=contourLevels,
                               masked_values=[1.0e30]
                              )
        plt.clabel(cs1, fmt=r"%4.2f")
        plt.fill(poly_pts[:,0], poly_pts[:,1], alpha=0.2)
        cs2 = mx.contour_array(combined_conc_3d[0], 
        ...
        for x in np.linspace(0.25, 9.75, 11):
            ax.arrow(x, 5.25, 0., -0.25, head_width=0.05, head_length=0.1, fc='b', ec='b', clip_on=False, length_includes_head=True)
        
        ax.arrow(0.25, 5.25, 9.5, 0., fc='b', ec='b', clip_on=False)
        ax.annotate('Infiltration = 0.10 m/day', xy=(0.025, 1.06), xycoords='axes fraction', clip_on=False)
        for x in np.linspace(3.75, 6.25, 6):
            ax.arrow(x, 5.5, 0., -0.5, head_width=0.05, head_length=0.1, fc='r', ec='r', clip_on=False, length_includes_head=True)
        
        ax.arrow(3.75, 5.5, 2.5, 0., fc='b', ec='r', clip_on=False)
        ax.annotate('Concentration = 1.00 mg/L', xy=(0.38, 1.12), xycoords='axes fraction', clip_on=False)
