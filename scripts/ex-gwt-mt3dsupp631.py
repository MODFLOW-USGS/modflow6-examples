...
longitudinal_dispersivity = 1. # Longitudinal dispersivity ($m$)
porosity = 0.37 # Porosity of mobile domain (unitless)
zero_order_decay = -2.e-3 # Zero-order production rate ($mg/L d^{-1}$)
source_duration = 160. # Source duration ($d$)
...
def build_mf6gwf(sim_folder):
    ...
    wel_spd = {0: [[(0, 0, 0), specific_discharge * delc * delr * top, 1.0]],
...
def build_mt3dms(sim_folder, modflowmodel):
    ...
    rct = flopy.mt3d.Mt3dRct(mt,
                             ...
                             ireact=ireact,
                             rc1=rc1)
    ssm_spd = {0:[0, 0, 0, 1., 2], 1:[0, 0, 0, 0., 2]}
...
def plot_results(sims, idx):
    if config.plotModel:
        ...
        axs.plot(mt3dms_ra['time'], mt3dms_ra[colname], linestyle='--',
                 label='MT3DMS')
